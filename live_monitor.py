...
def monitor_camera(camera_id, camera_url):
    ...
    while True:
        ...
        if detect_motion(frame_gpu.download()):
            cv2.imwrite(F"{camera_id}_{frame_counter}.png", np.hstack([original_frame, cv2.cvtColor(fgmask, cv2.COLOR_GRAY2BGR), cv2.cvtColor(despeckled, cv2.COLOR_GRAY2BGR)]))
