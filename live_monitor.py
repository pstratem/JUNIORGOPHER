...
def despeckle(threshold_image):
    kernel = cv.getStructuringElement(cv.MORPH_RECT, (threshold_image.shape[0] * 0.003, thresold_image.shape[1] * 0.003))
...
def monitor_camera(camera_id, camera_url):
    ...
    while True:
        ...
        if (capture.get(cv.CAP_PROP_POS_FRAMES) % (frame_rate / 5)) == 0:
            ...
            retval = cv.imwrite(os.path.join(camera_fgmasks_path, str(int(frame_time*1000)) + "despeckle" + ".jpg"), despeckled_image)
