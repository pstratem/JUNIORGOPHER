...
for camera_id, camera_url in cameras:
    ...
    start_stop_daemon = F'/sbin/start-stop-daemon --start --background --make-pidfile --pidfile {pid_path}/{camera_id}.ffmpeg.pid --exec /usr/bin/ffmpeg -- -i {camera_url} -use_wallclock_as_timestamps 1 -c:v copy -an -f segment -strftime 1 -segment_time 60 -segment_format mp4 -movflags +faststart {camera_segment_path}/%s.mp4'
